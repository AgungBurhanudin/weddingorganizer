...
if ($aturan == 'ADMINM1' || $aturan == 'ADMINM2' || $aturan == 'ADMINM3' || $aturan == 'ADMINFINANCE' || $aturan == 'FINANCEM1' || $aturan == 'FINANCEM2' || $aturan == 'FINANCEM3' || $aturan == 'M1M2' || $aturan == 'M1M3' || $aturan == 'M2M2' || $aturan == 'M2M3' || $aturan == 'M3M3') {
    ...
    if ($action == 'inq') {
        $response = array(
            'response_code' => '0000',
            'saldo' => $saldo_member,
            'response_message' => "Apakah anda ingin melakukan TRANSFER Saldo ke Saldo Sekolah $nama_add Rp. " . $fungsi->rupiah($amount_add) . " ?"
        );
    } elseif ($action == 'exec') {
        ...
        if ($saldo_member < $amount_add) {
            ...
            $sql_kurang_saldo = "update tbl_member_account set saldo_sekolah = saldo_sekolah + $amount_minus,last_trx='$jlast_trx',last_amount='$amount_minus',last_reff='$ref',last_fee='{}',last_lembar=1 where noid = '$noid' returning saldo_sekolah;";
            $saldo_noid = $db->singleRow($sql_kurang_saldo);
            $saldo_last = $saldo_noid->saldo_sekolah;
            ...
            $sql_tambah_saldo = "update tbl_member_account set saldo_sekolah = saldo_sekolah + $amount_add,last_trx='$jlast_trx_noid',last_amount=$amount_add,last_reff='$ref',last_fee='{}',last_lembar=1 where noid = '$noid_add' returning saldo_sekolah;";
            $saldo_add = $db->singleRow($sql_tambah_saldo);
            $saldo_add_last = $saldo_add->saldo_sekolah;
