...
$waktu = $arr_trx->waktu;
$noid = $arr_trx->noid;
$judul_struk = 'STRUK PEMBAYARAN TAGIHAN KARTU HALO';
$idpel = $jst->idpel;
$nama = $jst->nama;
$lembar = $jst->lembar;
$ref = $jst->reff;
...
0x1B@0x0 LAYANAN    : ' . $fungsi->spasi(27, $arr_trx->product_detail) . 'LAYANAN     : ' . $arr_trx->product_detail. '
0x0 NO TLP     : ' . $fungsi->spasi(27, substr($idpel, 0,strlen($idpel)-4) . "####") . 'NO TELEPON  : ' . substr($idpel, 0,strlen($idpel)-4) . "####" . '
...
0x0';
    ...
    . "\n--------------------------------"
    . "\nTANGGAL  : " . $waktu
    . "\nNO KARTU : " . substr($idpel, 0,strlen($idpel)-4) . "####"
    . "\nNAMA PLG : " . $nama
    . "\nJML TAG  : Rp." . $total_tagihan
    . "\n================================"
    . "\n\n\n";
	
} elseif ($printType == 'PDF') {
    $isi_tbl = '';
    $isi_tbl = "LAYANAN>: $idpel^3> ^3<"
            . "NO TELEPON>: $nama^3> ^3<"
            . "NAMA PLG>: $nama^3> ^3<"
