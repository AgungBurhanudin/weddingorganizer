...
class REPORT_PAGINGPOS extends Resources\Controller {
    ...
    //proteksi ip
    public function act($tipe_request, $interface) {
...
//        $param = json_encode($_POST);
        ...
        if (strpos($hak_akses, $tipe_member) !== false || strlen($hak_akses) < 2) {
            include "REPORT_PAGING/$tipe_request.php";
            $wLog->writeLog($interface, 'RPAGING_'.$tipe_request, json_encode($result));
        } else {
            $error->menuTidakBerhak($saldo_member);
        }
