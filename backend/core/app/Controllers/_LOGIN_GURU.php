            ...
            if ($status == 1) {
                if ($passwd == $this->password) {
                    if ($aplikasiid == $aplikasiid_add) {
                        //login sukses
                        $sql_sekolah             = "SELECT b.kode_sekolah as id_sekolah,b.nama_sekolah FROM tbl_guru a LEFT JOIN tbl_sekolah b ON a.id_sekolah = b.id WHERE nohp = '$nohp_email'";
                        $data_sekolah            = $db->multipleRow($sql_sekolah);
