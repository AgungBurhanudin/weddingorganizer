...
$sql = "select row_number() over (ORDER BY id) as no,id,noid,nama,'process' as fee_m2, 'process' as fee_total_m2, 'process' as fee_total_m3 "
        . "from tbl_member_account "
        . "where tipe = 'M3' $filter_search $filter_def_noid order by $sort $order limit $rows offset $offset;";
$sql_count = "select count(id) as jml from tbl_member_account where tipe = 'M3' $filter_search $filter_def_noid;";
