...
$sql = "select id,waktu::timestamp(0),noid,product,product_detail,idpel,"
        . "CASE WHEN response_code = '0000' THEN idpel_name ELSE substring(response_message from 1 for 40) END as idpel_name,"
        . "trace_id,reff,"
        . "CASE WHEN status=0 THEN 'INQUIRY' "
        . "WHEN status=1 THEN 'SUKSES' "
        . "WHEN status=2 THEN 'GAGAL' "
        . "WHEN status=3 THEN 'PENDING' "
        . "ELSE 'REFUND' END as status,"
        . "tagihan,admin_bank,total_tagihan from log_channel_trx "
        . "where 1=1 $filter_search $filter_def_noid order by $sort $order limit $rows offset $offset;";
$sql_count = "select count(id) as jml from log_channel_trx where 1=1 $filter_search $filter_def_noid;";
