...
//Margin Pusat dan data produk
    ...
    if (isset($result[0]->id)) {
        foreach ($result as $fetch) {
            ...
            $data[$i]['admin_bank'] = $fetch->admin_bank;
            if ($noid == $noid_add) { // m3
                $data[$i]['harga_m3'] = $fetch->admin_bank - ($fee_m1 + $fee_m2 + $fee_m3 + $fetch->hpp);
            } elseif ($tipe_member == 'M2') {
                $data[$i]['harga_m2'] = $fetch->admin_bank - ($fee_m1 + $fee_m2 + $fetch->hpp);
                $data[$i]['harga_m3'] = $fetch->admin_bank - ($fee_m1 + $fee_m2 + $fee_m3 + $fetch->hpp);
                $data[$i]['fee_m3'] = $fee_m3;
            } else {
                $data[$i]['harga_m1'] = $fetch->admin_bank - ($fee_m1 + $fetch->hpp);
                if($jenis_member == 2){
                    $data[$i]['harga_hpp'] = $fetch->admin_bank - ($fetch->hpp);
